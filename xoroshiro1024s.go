...
type Xoroshiro1024s struct {
	s [16]uint64
	p int
}

func NewXoroshiro1024s(seed int64) *Xoroshiro1024s {
	x := Xoroshiro1024s{}
	x.Seed(seed)
	return &x
}

func (x Xoroshiro1024s) State() []byte {
	s := make([]byte, 128)
...
func (x *Xoroshiro1024s) SetState(state []byte) {
...
func (x *Xoroshiro1024s) Seed(seed int64) {
...
func (x *Xoroshiro1024s) Uint64() uint64 {
	q := x.p
	x.p = (x.p + 1) & 15
	s0 := x.s[x.p]
	s15 := x.s[q]
...
func (x *Xoroshiro1024s) Int64() int64 {
	return unsafeUint64ToInt64(x.Uint64())
}

func (x *Xoroshiro1024s) Int63() int64 {
	return int64(x.Uint64() & (1<<63 - 1))
}

func (x *Xoroshiro1024s) Float64() float64 {
	return math.Float64frombits(0x3ff<<52|x.Uint64()>>12) - 1.0
}

// Call Uint64() * 2^512
func (x *Xoroshiro1024s) Jump() {
	var (
		jump = [...]uint64{
...
// Call Uint64() * 2^768
func (x *Xoroshiro1024s) LongJump() {
	var (
		jump = [...]uint64{
...
func (x Xoroshiro1024s) String() string {
	return fmt.Sprintf("%064x", x.State())
}

func (x Xoroshiro1024s) GoString() string {
	return "xrand.Xoshiro1024s{state:\"" + x.String() + "\"}"
}
